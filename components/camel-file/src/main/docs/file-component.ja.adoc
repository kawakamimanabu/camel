...
| *fileName* （共通） | ファイル名を動的に設定するための File Language のような Expression。コンシューマではファイル名のフィルタとして使用される。プロデューサでは書き込むためのファイル名を評価するために使用される。Expression が設定された場合、CamelFileName ヘッダより優先される。（Note: ヘッダそのものが Expression であり得る）Expression のオプションは String と Expression 型の両方をサポートしている。Expression が文字型の場合、File Language で評価される。Expression 型の場合、指定された Expression 型が使用される。- これにより、たとえば OGNL が使用できる。コンシューマにとってはファイル名をフィルタするために使用できる。よって File Language のシンタックスを用いて本日のファイルを処理するには次のようにする: mydata-\${date:now:yyyyMMdd}.txt プロデューサは CamelOverruleFileName ヘッダをサポートしている。これは既存の CamelFileName ヘッダより優先される; CamelOverruleFileName は 1 度だけ使用されるヘッダである。, and makes it easier as this avoids to temporary store CamelFileName and have to restore it afterwards. |  | String
