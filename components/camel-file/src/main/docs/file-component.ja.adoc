...
////
Camel はディレクトリ名で設定されたエンドポイントのみサポートしている。よって *directoryName* はディレクトリでなくてはならない。
単一のファイルのみを扱う場合は、たとえば `fileName=thefilename` のようにして *fileName* オプションが使用できる。
また、ディレクトリは `${ }` プレースホルダのような動的な Expression を含んではいけない。
