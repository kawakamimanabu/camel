...
////
ファイルコンポーネントは、
他の Camel コンポーネントにより処理されたファイル、もしくは他のコンポーネントからのメッセージがディスクに保存されるようにするためにファイルシステムへのアクセスを提供する。
...
////
*directoryName* は、ファイルディレクトリの場所を表している。

////
You can append query options to the URI in the following format,
`?option=value&option=value&...`
////
次の書式で URI にクエリオプションを追記できる。
`?option=value&option=value&...`
...
////
Camel は処理を開始するディレクトリ名が設定されたエンドポイントのみサポートしている。よって *directoryName* はディレクトリでなくてはならない。
単一のファイルのみを処理したい場合は、たとえば `fileName=thefilename` のようにして *fileName* オプションが使用できる。
また、処理を開始するディレクトリは `${ }` プレースホルダのような動的な Expression を含んではいけない。
繰り返しになるが、ファイル名の動的な部分を指定するには `fileName` オプションを使用すること。
...
////
*他のアプリケーションで同時に書き込まれているファイルの読み込みを避けること*
...
////
JDK File IO API は、他のアプリケーションが、ファイルに対して同時に書き込んだりコピーしているかを検知するには、少々制限がある。
また OS プラットフォームにより実装が異なる可能性がある。Camel がファイルの処理を開始する際には、他のプロセスによってロックされていないことを仮定する。
よって、環境により必要な調査を自分で行う必要がある。このために Camel は異なる `readLock` オプションと `doneFileName` オプションを提供している。
以下のセクションを参照のこと。<<File2-Consumingfilesfromfolderswhereothersdropfilesdirectly, 他のアプリケーションがファイルを直接削除しているフォルダからファイルを読み込む>>。
...
////
ファイルエンドポイントは URI シンタックスを用いて設定される。
...
|===
| 名前 | 説明 | デフォルト値 | 型
| *directoryName* | *必須* 開始ディレクトリ |  | File
|===
...
|===
| 名前 | 説明 | デフォルト値 | 型
| *charset* （共通） | This option is used to specify the encoding of the file. You can use this on the consumer, to specify the encodings of the files, which allow Camel to know the charset it should load the file content in case the file content is being accessed. Likewise when writing a file, you can use this option to specify which charset to write the file as well. Do mind that when writing the file Camel may have to read the message content into memory to be able to convert the data into the configured charset, so do not use this if you have big messages. |  | String
| *doneFileName* （共通） | Producer: If provided, then Camel will write a 2nd done file when the original file has been written. The done file will be empty. This option configures what file name to use. Either you can specify a fixed name. Or you can use dynamic placeholders. The done file will always be written in the same folder as the original file. Consumer: If provided, Camel will only consume files if a done file exists. This option configures what file name to use. Either you can specify a fixed name. Or you can use dynamic placeholders.The done file is always expected in the same folder as the original file. Only \${file.name} and \${file.name.noext} is supported as dynamic placeholders. |  | String
| *fileName* （共通） | ファイル名を動的に設定するための File Language のような式。コンシューマではファイル名のフィルタとして使用される。プロデューサでは書き込むためのファイル名を評価するために使用される。式が設定された場合、CamelFileName ヘッダより優先される。（Note: ヘッダそのものが式であり得る）式のオプションは String と式型の両方をサポートしている。式が文字型の場合、File Language で評価される。式型の場合、指定された式型が使用される。- これにより、たとえば OGNL が使用できる。コンシューマにとってはファイル名をフィルタするために使用できる。よって File Language のシンタックスを用いて本日のファイルを処理するには次のようにする: mydata-\${date:now:yyyyMMdd}.txt プロデューサは CamelOverruleFileName ヘッダをサポートしている。これは既存の CamelFileName ヘッダより優先される; CamelOverruleFileName は 1 度だけ使用されるヘッダで、CamelFileName を一時的に保存し後でリストアしなくてもよいので使い易い。 |  | String
| *bridgeErrorHandler* （コンシューマ） | Allows for bridging the consumer to the Camel routing Error Handler, which mean any exceptions occurred while the consumer is trying to pickup incoming messages, or the likes, will now be processed as a message and handled by the routing Error Handler. By default the consumer will use the org.apache.camel.spi.ExceptionHandler to deal with exceptions, that will be logged at WARN or ERROR level and ignored. | false | boolean
| *delete* （コンシューマ） | If true, the file will be deleted after it is processed successfully. | false | boolean
| *moveFailed* （コンシューマ） | Sets the move failure expression based on Simple language. For example, to move files into a .error subdirectory use: .error. Note: When moving the files to the fail location Camel will handle the error and will not pick up the file again. |  | String
| *noop* （コンシューマ） | If true, the file is not moved or deleted in any way. This option is good for readonly data, or for ETL type requirements. If noop=true, Camel will set idempotent=true as well, to avoid consuming the same files over and over again. | false | boolean
| *preMove* （コンシューマ） | 処理前にファイル名を動的に設定するために使用される（File Language のような）式。たとえば処理中のファイルを order ディレクトリに移動する場合は、この値を order に設定する。 |  | String
| *preSort* （コンシューマ） | When pre-sort is enabled then the consumer will sort the file and directory names during polling, that was retrieved from the file system. You may want to do this in case you need to operate on the files in a sorted order. The pre-sort is executed before the consumer starts to filter, and accept files to process by Camel. This option is default=false meaning disabled. | false | boolean
| *recursive* （コンシューマ） | If a directory, will look for files in all the sub-directories as well. | false | boolean
| *sendEmptyMessageWhenIdle* （コンシューマ） | If the polling consumer did not poll any files, you can enable this option to send an empty message (no body) instead. | false | boolean
| *directoryMustExist* （コンシューマ） | Similar to the startingDirectoryMustExist option but this applies during polling (after starting the consumer). | false | boolean
| *exceptionHandler* （コンシューマ） | To let the consumer use a custom ExceptionHandler. Notice if the option bridgeErrorHandler is enabled then this option is not in use. By default the consumer will deal with exceptions, that will be logged at WARN or ERROR level and ignored. |  | ExceptionHandler
| *exchangePattern* （コンシューマ） | Sets the exchange pattern when the consumer creates an exchange. |  | ExchangePattern
| *extendedAttributes* （コンシューマ） | To define which file attributes of interest. Like posix:permissions,posix:owner,basic:lastAccessTime, it supports basic wildcard like posix:, basic:lastAccessTime |  | String
| *inProgressRepository* （コンシューマ） | A pluggable in-progress repository org.apache.camel.spi.IdempotentRepository. The in-progress repository is used to account the current in progress files being consumed. By default a memory based repository is used. |  | IdempotentRepository
| *localWorkDirectory* （コンシューマ） | When consuming, a local work directory can be used to store the remote file content directly in local files, to avoid loading the content into memory. This is beneficial, if you consume a very big remote file and thus can conserve memory. |  | String
| *onCompletionException Handler* （コンシューマ） | To use a custom org.apache.camel.spi.ExceptionHandler to handle any thrown exceptions that happens during the file on completion process where the consumer does either a commit or rollback. The default implementation will log any exception at WARN level and ignore. |  | ExceptionHandler
| *pollStrategy* （コンシューマ） | A pluggable org.apache.camel.PollingConsumerPollingStrategy allowing you to provide your custom implementation to control error handling usually occurred during the poll operation before an Exchange have been created and being routed in Camel. |  | PollingConsumerPollStrategy
| *probeContentType* （コンシューマ） | Whether to enable probing of the content type. If enable then the consumer uses Files#probeContentType(java.nio.file.Path) to determine the content-type of the file, and store that as a header with key Exchange#FILE_CONTENT_TYPE on the Message. | false | boolean
| *processStrategy* （コンシューマ） | A pluggable org.apache.camel.component.file.GenericFileProcessStrategy allowing you to implement your own readLock option or similar. Can also be used when special conditions must be met before a file can be consumed, such as a special ready file exists. If this option is set then the readLock option does not apply. |  | GenericFileProcessStrategy
| *startingDirectoryMustExist* （コンシューマ） | Whether the starting directory must exist. Mind that the autoCreate option is default enabled, which means the starting directory is normally auto created if it doesn't exist. You can disable autoCreate and enable this to ensure the starting directory must exist. Will thrown an exception if the directory doesn't exist. | false | boolean
| *startingDirectoryMustHave Access* （コンシューマ） | Whether the starting directory has access permissions. Mind that the startingDirectoryMustExist parameter must be set to true in order to verify that the directory exists. Will thrown an exception if the directory doesn't have read and write permissions. | false | boolean
| *appendChars* （プロデューサ） | Used to append characters (text) after writing files. This can for example be used to add new lines or other separators when writing and appending to existing files. To specify new-line (slash-n or slash-r) or tab (slash-t) characters then escape with an extra slash, eg slash-slash-n |  | String
| *fileExist* （プロデューサ） a| すでに同名のファイルが存在した場合にどうすべきかを指定する。Override がデフォルトで既存のファイルを置換する。

 - Append - 中身を既存のファイルに追記する。
 
 - Fail - ファイルが既存であることを示す GenericFileOperationException をスローする。
 
 - Ignore - すべて OK であることを仮定して、発生する問題を無視し、既存のファイルを上書きしない。
 
 - Move - option requires to use the moveExisting option to be configured as well. The option eagerDeleteTargetFile can be used to control what to do if an moving the file, and there exists already an existing file, otherwise causing the move operation to fail. The Move option will move any existing files, before writing the target file.
 
 - TryRename is only applicable if tempFileName option is in use. This allows to try renaming the file from the temporary name to the actual name, without doing any exists check. This check may be faster on some file systems and especially FTP servers. | Override | GenericFileExist

| *flatten* （プロデューサ） | Flatten is used to flatten the file name path to strip any leading paths, so it's just the file name. This allows you to consume recursively into sub-directories, but when you eg write the files to another directory they will be written in a single directory. Setting this to true on the producer enforces that any file name in CamelFileName header will be stripped for any leading paths. | false | boolean
| *jailStartingDirectory* （プロデューサ） | Used for jailing (restricting) writing files to the starting directory (and sub) only. This is enabled by default to not allow Camel to write files to outside directories (to be more secured out of the box). You can turn this off to allow writing files to directories outside the starting directory, such as parent or root folders. | true | boolean
| *lazyStartProducer* （プロデューサ） | Whether the producer should be started lazy (on the first message). By starting lazy you can use this to allow CamelContext and routes to startup in situations where a producer may otherwise fail during starting and cause the route to fail being started. By deferring this startup to be lazy then the startup failure can be handled during routing messages via Camel's routing error handlers. Beware that when the first message is processed then creating and starting the producer may take a little time and prolong the total processing time of the processing. | false | boolean
| *moveExisting* （プロデューサ） | Expression (such as File Language) used to compute file name to use when fileExist=Move is configured. To move files into a backup subdirectory just enter backup. This option only supports the following File Language tokens: file:name, file:name.ext, file:name.noext, file:onlyname, file:onlyname.noext, file:ext, and file:parent. Notice the file:parent is not supported by the FTP component, as the FTP component can only move any existing files to a relative directory based on current dir as base. |  | String
| *tempFileName* （プロデューサ） | The same as tempPrefix option but offering a more fine grained control on the naming of the temporary filename as it uses the File Language. The location for tempFilename is relative to the final file location in the option 'fileName', not the target directory in the base uri. For example if option fileName includes a directory prefix: dir/finalFilename then tempFileName is relative to that subdirectory dir. |  | String
| *tempPrefix* （プロデューサ） | This option is used to write the file using a temporary name and then, after the write is complete, rename it to the real name. Can be used to identify files being written and also avoid consumers (not using exclusive read locks) reading in progress files. Is often used by FTP when uploading big files. |  | String
| *allowNullBody* （プロデューサ） | Used to specify if a null body is allowed during file writing. If set to true then an empty file will be created, when set to false, and attempting to send a null body to the file component, a GenericFileWriteException of 'Cannot write null body to file.' will be thrown. If the fileExist option is set to 'Override', then the file will be truncated, and if set to append the file will remain unchanged. | false | boolean
| *chmod* （プロデューサ） | Specify the file permissions which is sent by the producer, the chmod value must be between 000 and 777; If there is a leading digit like in 0755 we will ignore it. |  | String
| *chmodDirectory* （プロデューサ） | Specify the directory permissions used when the producer creates missing directories, the chmod value must be between 000 and 777; If there is a leading digit like in 0755 we will ignore it. |  | String
| *eagerDeleteTargetFile* （プロデューサ） | Whether or not to eagerly delete any existing target file. This option only applies when you use fileExists=Override and the tempFileName option as well. You can use this to disable (set it to false) deleting the target file before the temp file is written. For example you may write big files and want the target file to exists during the temp file is being written. This ensure the target file is only deleted until the very last moment, just before the temp file is being renamed to the target filename. This option is also used to control whether to delete any existing files when fileExist=Move is enabled, and an existing file exists. If this option copyAndDeleteOnRenameFails false, then an exception will be thrown if an existing file existed, if its true, then the existing file is deleted before the move operation. | true | boolean
| *forceWrites* （プロデューサ） | Whether to force syncing writes to the file system. You can turn this off if you do not want this level of guarantee, for example if writing to logs / audit logs etc; this would yield better performance. | true | boolean
| *keepLastModified* （プロデューサ） | Will keep the last modified timestamp from the source file (if any). Will use the Exchange.FILE_LAST_MODIFIED header to located the timestamp. This header can contain either a java.util.Date or long with the timestamp. If the timestamp exists and the option is enabled it will set this timestamp on the written file. Note: This option only applies to the file producer. You cannot use this option with any of the \ftp producers. | false | boolean
| *moveExistingFileStrategy* （プロデューサ） | Strategy (Custom Strategy) used to move file with special naming token to use when fileExist=Move is configured. By default, there is an implementation used if no custom strategy is provided |  | FileMoveExistingStrategy
| *autoCreate* （上級者向け） | Automatically create missing directories in the file's pathname. For the file consumer, that means creating the starting directory. For the file producer, it means the directory the files should be written to. | true | boolean
| *basicPropertyBinding* （上級者向け） | Whether the endpoint should use basic property binding (Camel 2.x) or the newer property binding with additional capabilities | false | boolean
| *bufferSize* （上級者向け） | Buffer size in bytes used for writing files (or in case of FTP for downloading and uploading files). | 131072 | int
| *copyAndDeleteOnRenameFail* （上級者向け） | Whether to fallback and do a copy and delete file, in case the file could not be renamed directly. This option is not available for the FTP component. | true | boolean
| *renameUsingCopy* （上級者向け） | Perform rename operations using a copy and delete strategy. This is primarily used in environments where the regular rename operation is unreliable (e.g. across different file systems or networks). This option takes precedence over the copyAndDeleteOnRenameFail parameter that will automatically fall back to the copy and delete strategy, but only after additional delays. | false | boolean
| *synchronous* （上級者向け） | Sets whether synchronous processing should be strictly used, or Camel is allowed to use asynchronous processing (if supported). | false | boolean
| *antExclude* （フィルタ） | Ant style filter exclusion. If both antInclude and antExclude are used, antExclude takes precedence over antInclude. Multiple exclusions may be specified in comma-delimited format. |  | String
| *antFilterCaseSensitive* （フィルタ） | Sets case sensitive flag on ant filter | true | boolean
| *antInclude* （フィルタ） | Ant style filter inclusion. Multiple inclusions may be specified in comma-delimited format. |  | String
| *eagerMaxMessagesPerPoll* （フィルタ） | Allows for controlling whether the limit from maxMessagesPerPoll is eager or not. If eager then the limit is during the scanning of files. Where as false would scan all files, and then perform sorting. Setting this option to false allows for sorting all files first, and then limit the poll. Mind that this requires a higher memory usage as all file details are in memory to perform the sorting. | true | boolean
| *exclude* （フィルタ） | Is used to exclude files, if filename matches the regex pattern (matching is case in-senstive). Notice if you use symbols such as plus sign and others you would need to configure this using the RAW() syntax if configuring this as an endpoint uri. See more details at configuring endpoint uris |  | String
| *filter* （フィルタ） | Pluggable filter as a org.apache.camel.component.file.GenericFileFilter class. Will skip files if filter returns false in its accept() method. |  | GenericFileFilter
| *filterDirectory* （フィルタ） | Filters the directory based on Simple language. For example to filter on current date, you can use a simple date pattern such as \${date:now:yyyMMdd} |  | String
| *filterFile* （フィルタ） | Filters the file based on Simple language. For example to filter on file size, you can use \${file:size} 5000 |  | String
| *idempotent* （フィルタ） | Option to use the Idempotent Consumer EIP pattern to let Camel skip already processed files. Will by default use a memory based LRUCache that holds 1000 entries. If noop=true then idempotent will be enabled as well to avoid consuming the same files over and over again. | false | Boolean
| *idempotentKey* （フィルタ） | To use a custom idempotent key. By default the absolute path of the file is used. You can use the File Language, for example to use the file name and file size, you can do: idempotentKey=\${file:name}-\${file:size} |  | String
| *idempotentRepository* （フィルタ） | A pluggable repository org.apache.camel.spi.IdempotentRepository which by default use MemoryMessageIdRepository if none is specified and idempotent is true. |  | IdempotentRepository
| *include* （フィルタ） | Is used to include files, if filename matches the regex pattern (matching is case in-sensitive). Notice if you use symbols such as plus sign and others you would need to configure this using the RAW() syntax if configuring this as an endpoint uri. See more details at configuring endpoint uris |  | String
| *maxDepth* （フィルタ） | The maximum depth to traverse when recursively processing a directory. | 2147483647 | int
| *maxMessagesPerPoll* （フィルタ） | To define a maximum messages to gather per poll. By default no maximum is set. Can be used to set a limit of e.g. 1000 to avoid when starting up the server that there are thousands of files. Set a value of 0 or negative to disabled it. Notice: If this option is in use then the File and FTP components will limit before any sorting. For example if you have 100000 files and use maxMessagesPerPoll=500, then only the first 500 files will be picked up, and then sorted. You can use the eagerMaxMessagesPerPoll option and set this to false to allow to scan all files first and then sort afterwards. |  | int
| *minDepth* （フィルタ） | The minimum depth to start processing when recursively processing a directory. Using minDepth=1 means the base directory. Using minDepth=2 means the first sub directory. |  | int
| *move* （フィルタ） | Expression (such as Simple Language) used to dynamically set the filename when moving it after processing. To move files into a .done subdirectory just enter .done. |  | String
| *exclusiveReadLockStrategy* (lock) | Pluggable read-lock as a org.apache.camel.component.file.GenericFileExclusiveReadLockStrategy implementation. |  | GenericFileExclusiveReadLockStrategy
| *readLock* （ロック） a| コンシューマで使用される。ファイルに対して排他的な読み込みロックを保持している（たとえばファイルが処理中だったり、書き込まれている途中ではない）場合にファイルをポーリングするために使用される。Camel はファイルのロックが付与されるまで待機する。このオプションは以下のビルトインの方法がある:

 - none - 使用中の読み取りロックが無い

 - markerFile - Camel が（ファイル名 camelLock という名前で）マーカファイルを作成し、それに対してロックを保持する。FTP コンポーネントに対しては本オプションは利用できない。

 - changed - 本オプションはファイルが現在コピーされているか否かを検知するために、ファイルの長さや更新時刻を検知している。検知するために短くて 1 秒であるため、他のオプションのように早くファイルを読み込めないが信頼できる。というのも JDK IO API は、ファイルが他のプロセスにより使われているか否かをいつも検知できるとは限らないからである。readLockCheckInterval オプションがチェックの頻度を設定するために使用される。

 - fileLock - java.nio.channels.FileLock を使用するためのオプションである。本オプションは Windows OS と FTP コンポーネントに対しては利用できない。ファイルシステムが分散ファイルロックをサポートしていない場合は、マウントもしくは共有によってリモートのファイルにアクセスするこのやり方は避けるべきである。

 - rename - リネームは排他的読み取りロックが取得できるかどうかのテストとしてファイルをリネームしようとする。

 - idempotent - （ファイルコンポーネントのみ）読み取りロックとして冪等性リポジトリを使用する。冪等性リポジトリの実装がクラスタリングをサポートしている場合は、クラスタリングをサポートする読み取りロックが使用できる。

 - idempotent-changed - （ファイルコンポーネントのみ）本オプションは冪等性リポジトリと changed オプションを用いて組み合わせた読み取りロックとして使用する。冪等性リポジトリの実装がクラスタリングをサポートしている場合は、クラスタリングをサポートする読み取りロックが使用できる。

 - idempotent-rename - （ファイルコンポーネントのみ）本オプションは冪等性リポジトリと rename オプションを用いて組み合わせた読み取りロックとして使用する。 冪等性リポジトリの実装がクラスタリングをサポートしている場合は、クラスタリングをサポートする読み取りロックが使用できる。
 
Notice: 様々な読み取りロックは、すべてがクラスタモードで機能するわけではない。異なるノード上のコンシューマが同時にアクセスした場合は、共有ファイルシステム上で同一ファイルに対して競合する。空のマーカファイルを作成して操作を行う markerFile はクラスタで動作することを保証しない。fileLock が動作するには分散ファイルロックがサポートされている必要がある。冪等性読み取りロックは、Hazelcast や Infinispan コンポーネントのような冪等性リポジトリがクラスタリングをサポートしている場合に、クラスタリングをサポートしている。 | none | String
...
| *readLockDeleteOrphanLock Files* （ロック） | Whether or not read lock with marker files should upon startup delete any orphan read lock files, which may have been left on the file system, if Camel was not properly shutdown (such as a JVM crash). If turning this option to false then any orphaned lock file will cause Camel to not attempt to pickup that file, this could also be due another node is concurrently reading files from the same shared directory. | true | boolean
| *readLockIdempotentRelease Async* （ロック） | Whether the delayed release task should be synchronous or asynchronous. See more details at the readLockIdempotentReleaseDelay option. | false | boolean
| *readLockIdempotentRelease AsyncPoolSize* （ロック） | The number of threads in the scheduled thread pool when using asynchronous release tasks. Using a default of 1 core threads should be sufficient in almost all use-cases, only set this to a higher value if either updating the idempotent repository is slow, or there are a lot of files to process. This option is not in-use if you use a shared thread pool by configuring the readLockIdempotentReleaseExecutorService option. See more details at the readLockIdempotentReleaseDelay option. |  | int
| *readLockIdempotentRelease Delay* （ロック） | Whether to delay the release task for a period of millis. This can be used to delay the release tasks to expand the window when a file is regarded as read-locked, in an active/active cluster scenario with a shared idempotent repository, to ensure other nodes cannot potentially scan and acquire the same file, due to race-conditions. By expanding the time-window of the release tasks helps prevents these situations. Note delaying is only needed if you have configured readLockRemoveOnCommit to true. |  | int
| *readLockIdempotentRelease ExecutorService* （ロック） | To use a custom and shared thread pool for asynchronous release tasks. See more details at the readLockIdempotentReleaseDelay option. |  | ScheduledExecutorService
| *readLockLoggingLevel* （ロック） | Logging level used when a read lock could not be acquired. By default a DEBUG is logged. You can change this level, for example to OFF to not have any logging. This option is only applicable for readLock of types: changed, fileLock, idempotent, idempotent-changed, idempotent-rename, rename. | DEBUG | LoggingLevel
| *readLockMarkerFile* （ロック） | Whether to use marker file with the changed, rename, or exclusive read lock types. By default a marker file is used as well to guard against other processes picking up the same files. This behavior can be turned off by setting this option to false. For example if you do not want to write marker files to the file systems by the Camel application. | true | boolean
| *readLockMinAge* （ロック） | This option is applied only for readLock=changed. It allows to specify a minimum age the file must be before attempting to acquire the read lock. For example use readLockMinAge=300s to require the file is at last 5 minutes old. This can speedup the changed read lock as it will only attempt to acquire files which are at least that given age. | 0 | long
| *readLockMinLength* （ロック） | This option is applied only for readLock=changed. It allows you to configure a minimum file length. By default Camel expects the file to contain data, and thus the default value is 1. You can set this option to zero, to allow consuming zero-length files. | 1 | long
| *readLockRemoveOnCommit* （ロック） | This option is applied only for readLock=idempotent. It allows to specify whether to remove the file name entry from the idempotent repository when processing the file is succeeded and a commit happens. By default the file is not removed which ensures that any race-condition do not occur so another active node may attempt to grab the file. Instead the idempotent repository may support eviction strategies that you can configure to evict the file name entry after X minutes - this ensures no problems with race conditions. See more details at the readLockIdempotentReleaseDelay option. | false | boolean
| *readLockRemoveOnRollback* （ロック） | This option is applied only for readLock=idempotent. It allows to specify whether to remove the file name entry from the idempotent repository when processing the file failed and a rollback happens. If this option is false, then the file name entry is confirmed (as if the file did a commit). | true | boolean
| *readLockTimeout* （ロック） | Optional timeout in millis for the read-lock, if supported by the read-lock. If the read-lock could not be granted and the timeout triggered, then Camel will skip the file. At next poll Camel, will try the file again, and this time maybe the read-lock could be granted. Use a value of 0 or lower to indicate forever. Currently fileLock, changed and rename support the timeout. Notice: For FTP the default readLockTimeout value is 20000 instead of 10000. The readLockTimeout value must be higher than readLockCheckInterval, but a rule of thumb is to have a timeout that is at least 2 or more times higher than the readLockCheckInterval. This is needed to ensure that amble time is allowed for the read lock process to try to grab the lock before the timeout was hit. | 10000 | long
| *backoffErrorThreshold* （スケジューラ） | The number of subsequent error polls (failed due some error) that should happen before the backoffMultipler should kick-in. |  | int
| *backoffIdleThreshold* （スケジューラ） | The number of subsequent idle polls that should happen before the backoffMultipler should kick-in. |  | int
| *backoffMultiplier* （スケジューラ） | To let the scheduled polling consumer backoff if there has been a number of subsequent idles/errors in a row. The multiplier is then the number of polls that will be skipped before the next actual attempt is happening again. When this option is in use then backoffIdleThreshold and/or backoffErrorThreshold must also be configured. |  | int
| *delay* （スケジューラ） | Milliseconds before the next poll. You can also specify time values using units, such as 60s (60 seconds), 5m30s (5 minutes and 30 seconds), and 1h (1 hour). | 500 | long
| *greedy* （スケジューラ） | If greedy is enabled, then the ScheduledPollConsumer will run immediately again, if the previous run polled 1 or more messages. | false | boolean
| *initialDelay* （スケジューラ） | Milliseconds before the first poll starts. You can also specify time values using units, such as 60s (60 seconds), 5m30s (5 minutes and 30 seconds), and 1h (1 hour). | 1000 | long
| *repeatCount* （スケジューラ） | Specifies a maximum limit of number of fires. So if you set it to 1, the scheduler will only fire once. If you set it to 5, it will only fire five times. A value of zero or negative means fire forever. | 0 | long
| *runLoggingLevel* （スケジューラ） | The consumer logs a start/complete log line when it polls. This option allows you to configure the logging level for that. | TRACE | LoggingLevel
| *scheduledExecutorService* （スケジューラ） | Allows for configuring a custom/shared thread pool to use for the consumer. By default each consumer has its own single threaded thread pool. |  | ScheduledExecutorService
| *scheduler* （スケジューラ） | To use a cron scheduler from either camel-spring or camel-quartz component | none | String
| *schedulerProperties* （スケジューラ） | To configure additional properties when using a custom scheduler or any of the Quartz, Spring based scheduler. |  | Map
| *startScheduler* （スケジューラ） | Whether the scheduler should be auto started. | true | boolean
| *timeUnit* （スケジューラ） | Time unit for initialDelay and delay options. | MILLISECONDS | TimeUnit
| *useFixedDelay* （スケジューラ） | Controls if fixed delay or fixed rate is used. See ScheduledExecutorService in JDK for details. | true | boolean
| *shuffle* (sort) | To shuffle the list of files (sort in random order) | false | boolean
| *sortBy* (sort) | Built-in sort by using the File Language. Supports nested sorts, so you can have a sort by file name and as a 2nd group sort by modified date. |  | String
| *sorter* (sort) | Pluggable sorter as a java.util.Comparator class. |  | Comparator
|===
...
////
TIP: *ファイルプロデューサのデフォルトのふるまい* デフォルトで同名のファイルが存在した場合に、既存のファイルをオーバライドする。
...
|===
| 名前 | 説明 | デフォルト値 | 型
| *camel.component.file.basic-property-binding* | コンポーネントが基本的なプロパティバインディング(Camel 2.x)を使用するか付加的な機能を使用するより新しいプロパティバインディングを使用するかを指定する。 | false | Boolean
| *camel.component.file.bridge-error-handler* | コンシューマが Camel のルーティングエラーハンドラに渡すことを許可する。これはコンシューマがメッセージを受信しようとして例外が発生した場合に、メッセージとして処理されルーティングエラーハンドラに渡されることを意味する。デフォルトでは、コンシューマは例外を扱うのに org.apache.camel.spi.ExceptionHandler を使用し、WARN もしくは ERROR レベルでログ出力され、無視される。 | false | Boolean
| *camel.component.file.enabled* | file コンポーネントの auto configuration を可能にするように指定する。デフォルトで enabled になっている。 |  | Boolean
| *camel.component.file.lazy-start-producer* | （最初のメッセージで）遅延でプロデューサが開始されるか否かを指定する。遅延で開始することにより、CamelContext とルートがプロデューサが開始時に失敗する、もしくはルートが失敗するかもしれない状況において開始できるようになる。遅延で開始すると、開始時の失敗は Camel のルーティングエラーハンドラを介してメッセージがルーティングされて扱われる。最初のメッセージが処理されたときに、プロデューサを作成して開始するには少し時間が掛かり、全体の処理時間は長くなることに注意すること。 | false | Boolean
|===
...
////
移動や削除の操作はルーティングが完了した後（コマンドの後）で実行される:
よって `Exchange` の処理中では、ファイルは inbox フォルダに置かれたままとなる。
...
////
`inbox` フォルダにファイルがドロップされると、ファイルコンシューマがこれを検知して、新規の `FileExchange` を生成する。これは `handleOrder` ビーンにルーティングされる。
ビーンは `File` オブジェクトを処理する。この時点ではファイルは、まだ `inbox` フォルダにある。
ビーンの処理が終わるとルートは完了し、ファイルコンシューマはファイルの移動処理を行いファイルを `done` サブフォルダに移動する。
...
////
*move* と *preMove* オプションは、相対もしくは絶対のいずれかのディレクトリ名であると解釈される
（もし xref:manual::file-language.adoc[File Language] もしくは xref:manual::simple-language.adoc[Simple] の式を使用している場合、式の評価の結果は使用されるファイル名となる。たとえば以下のように設定したとする
...
////
このとき xref:manual::file-language.adoc[File Language] は使用されているファイル名を返却する）。
相対の場合は、ファイルが処理されたフォルダからサブフォルダとしてディレクトリが作成される。

////
By default, Camel will move consumed files to the `.camel` sub-folder
relative to the directory where the file was consumed.
////
デフォルトでは、Camel は、ファイルが処理されたディレクトリ配下の `.camel` サブフォルダに処理済みのファイルを移動する。

////
If you want to delete the file after processing, the route should be:
////
処理後にファイルを削除したい場合は、ルートを以下のようにする。
...
ファイルが処理される *前に* ファイルを移動する *pre* 移動オプションを紹介した。
これにより処理される前にサブフォルダに移動されるので、どのファイルがスキャンされたかをマークすることができる。
...
////
通常の move と *pre* オプションを組み合わせることができる。
...
////
この場合は、ファイルは処理中は `inprogress` フォルダにあり、処理後は `.done` フォルダに移動される。

////
== Fine grained control over Move and PreMove option
////
== Move と PreMove オプションの詳細な制御
...
////
*move* と *preMove* オプションは式ベースであり、ディレクトリと名前のパターンの高等な設定を行うために xref:manual::file-language.adoc[File Language] を使用する。+
実際 Camel は xref:manual::file-language.adoc[File Language] 式で入力したディレクトリ名を内部的に変換している。 `move=.done` このように入力した場合に、Camel は次のように変換する:
`\${file:parent}/.done/${file:onlyname}`
この変換はオプションの値に $\{ } を含めていないことを Camel が検知した場合にのみ発生する。よって $\{ } を含めた場合は、Camel は変換は *行わず* xref:manual::file-language.adoc[File Language] のすべてを利用できる。
...
////
本日日付でバックアップフォルダにファイルを移動させたい場合は、以下のようにする。
...
////
== 他のアプリケーションがファイルを直接削除しているフォルダからファイルを読み込む
...
////
他のアプリケーションがファイルを直接削除しているフォルダから、ファイルを読み込む場合は気を付けること。
異なる readLock オプションで、どれが自分のユースケースに合うかを確認すること。
ベストアプローチは、フォルダに対して書き込みを行ってから、削除するファイルがあるフォルダ内のファイルを move することである。
削除するファイルがあるフォルダ内のファイルに直接書き込みを行っている場合は、
changed オプションを使用することでファイルが同時に書き込み、またはコピーされているかを検出できる。
というのも、changed オプションは一定期間でファイルのサイズまたは変更を確認するアルゴリズムを使用しているためである。
他の readLock オプションは Java のファイル API に依存しており、このような検知を行うのはあまり得意ではない。
doneFileName を試してもよい。これはファイルが扱えるようになったら知らせるようなマーカファイル（done ファイル）を使用する。
...
////
これは done _file_ が目的のファイルと同じディレクトリ存在している場合に bar フォルダからファイルを処理するのみである。
Camel はファイルの処理が完了したときに _done file_ を自動的に削除する。
Camel は `noop=true` が設定されている場合には _done file_ を自動的に削除しない。
...
////
目的のファイルごとに 1 つの _done file_ を保持するのは普通のことである。
これは、1:1 の関係があることを意味する。このために、動的なプレースホルダを `doneFileName` オプション内で使用しなくてはならない。
現在 Camel は次の 2 つの動的なトークンをサポートしている: $\{ } で囲まれた `file:name` と `file:name.noext` である。
コンシューマはプレフィックスもしくはサフィックス（両方はない）として _done file_ のファイル名の静的な部分のみサポートしている。
...
////
たとえば以下の例では、_file name_.done のファイル名で done ファイルが存在した場合にファイルがポーリングされる。


////
* `hello.txt` - is the file to be consumed
////
* `hello.txt` - は処理されるファイルである
////
* `hello.txt.done` - is the associated done file
////
* `hello.txt.done` - は関連付けられた done ファイルである

////
You can also use a prefix for the done file, such as:
////
以下のようにして done ファイルに対してプレフィックスを用いることもできる。
...
////
* `ready-hello.txt` - は関連付けられた done ファイルである
...
////
ファイルに書きこんだ後に他のアプリケーションに対してファイル書き込みが完了したことを知らせるために _done__file_ をマーカとして書き込みたくなるかもしれない。
これを行うには、プロデューサエンドポイントの `doneFileName` オプションが利用できる。
...
target file.
////
目的のファイルとして同じディレクトリ内の `done` と名が付いたファイルを単純に作成する。
...
must be enclosed in $\{ }.
////
目的のファイルごとに 1 つの done ファイルを保持するのは普通のことである。
これは、1:1 の関係があることを意味する。このために、動的なプレースホルダを `doneFileName` オプション内で使用しなくてはならない。
現在 Camel は次の 2 つの動的なトークンをサポートしている: $\{ } で囲まれた `file:name` と `file:name.noext` である。
...
////
この例では、目的のファイルが `foo.txt` の場合に同じディレクトリに `done-foo.txt` の名前のファイルを作成する。
...
was `foo.txt` in the same directory as the target file.
////
この例では、目的のファイルが `foo.txt` の場合に同じディレクトリに `foo.txt.done` の名前のファイルを作成する。
...
////
この例では、目的のファイルが `foo.txt` の場合に同じディレクトリに `foo.done` の名前のファイルを作成する。
...
////
=== 動的なファイル名を用いてディレクトリから読み込んで、別のディレクトリに書き込む
...
`inputdir`.
////
ディレクトリを監視して、ファイルが格納されたらメッセージを生成する。
中身を `outputdir` にコピーして `inputdir` のファイルを削除する。
